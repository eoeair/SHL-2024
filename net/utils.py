...
class MyBatchNorm_stepCompute(nn.Module):  # quick framewise bn, the first few batches are only updated by themselves, but the later ones are shared
    ...
    def __init__(self, hidden_size, seq_len, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True):  # num_features
        ...
        if self.training:
            ...
            with torch.no_grad():
                self.running_mean[BN_start : input_t + BN_start] = (
                    exponential_average_factor * mean + (1 - exponential_average_factor) * self.running_mean[BN_start : input_t + BN_start]
                )
                # update running_var with unbiased var
                self.running_var[BN_start : input_t + BN_start] = (
                    exponential_average_factor * var * n / (n - 1) + (1 - exponential_average_factor) * self.running_var[BN_start : input_t + BN_start]
                )
        else:
